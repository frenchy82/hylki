...
T3 Code ships, adapted to libadwaita's color roles and renamed for Hylki
...
named colors and the reader documents both want.
...
# color. Anything here survives a regeneration, so put a reason next to it.
#
...
# The roles Hylki maps onto libadwaita colors (see src/theme.rs). Upstream
...
def to_hex(value: str) -> str:
    ...
    if not match:
        raise SystemExit(f"unhandled color: {value}")
...
def main() -> None:
    ...
    out = [
        ...
        "//!",
        "//! The colors are T3 Code's theme library (MIT, Copyright (c) 2026 T3",
        "//! Tools Inc.), converted from OKLCH to sRGB hex and renamed for Hylki.",
        "//! `theme.rs` maps them onto libadwaita's color roles. Edit the",
